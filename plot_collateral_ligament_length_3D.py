...
for i_num in range(len(ligament_list)):
    ...
    ax.plot_surface(Theta, Phi, Z, label=f'#{i_num}', color=colors[i_num], alpha=0.7)

# ax.set_xlabel(r'$\theta~\left[\mathrm{rad} \right]$')
# ax.set_ylabel(r'$\phi~\left[\mathrm{rad} \right]$')
