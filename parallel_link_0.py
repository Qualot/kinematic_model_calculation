...
attachment_tibia_forward.translate((0.03, 0.02, 0))
...
attachment_tibia_backward.translate((-0.035, 0.02, 0))
...
# Header
header = ["i", "theta", "phi", "q"] + [f"T#{tibia_num}->F#{j}" for j in range(n_femur)]
print(",".join(header))

for i in range(i_max):
    theta = (i_max - i) / i_max * np.pi / 4.0
    phi = phi_acute(theta)
    q = -theta + phi
