...
for i_num in range(len(ligament_list)):
    ...
    norm_vals = [norm_list[i_num].subs({d: 36, h: 36, theta: val, phi: 0.0}).evalf() for val in theta_vals]
    norm_vals_list.append(norm_vals)

# graph plot
for i_num in range(len(ligament_list)):
    style = '-' if i_num < 2 else 'dotted'  # 0,1 → solid ('-'), 2,3 → dashed ('--')
    #plt.plot(theta_vals, norm_vals_list[i_num], label=f'#{i_num}')
    plt.plot(theta_vals, norm_vals_list[i_num], label=f'{names_list[i_num]}', linestyle=style)
